...
class KMeans {
	...
	def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
		val mapping = points.groupBy((point: Point) => findClosest(point, means))
		means.map((mean: Point) => mean -> mapping.getOrElse(mean, Nil)).toMap
	}
	...
	def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
		oldMeans.map((mean: Point) => findAverage(mean, classified.getOrElse(mean, Nil)))
	}

	def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
		oldMeans.zip(newMeans).forall((means: (Point, Point)) => means._1.squareDistance(means._2) <= eta)
	}
